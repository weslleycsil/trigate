...
function serverHandler(request, response) {
    ...
    try {
        ...
        var matched = false;
        ['/static/', '/dev/', '/dist/', '/socket.io/', '/node_modules/canvas-designer/', '/admin/'].forEach(function(item) {
        ...
        try {
            ...
            if (filename.search(/static/g) === -1 && filename.search(/admin/g) === -1 && stats.isDirectory() && config.homePage === '/static/index.html') {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                if (filename.indexOf(resolveURL('/static/MultiRTC/')) !== -1) {
                    filename = filename.replace(resolveURL('/static/MultiRTC/'), '');
                    filename += resolveURL('/static/MultiRTC/index.html');
                ...
                } else if (filename.indexOf(resolveURL('/static/dashboard/')) !== -1) {
                    filename = filename.replace(resolveURL('/static/dashboard/'), '');
                    filename += resolveURL('/static/dashboard/index.html');
                } else if (filename.indexOf(resolveURL('/static/video-conference/')) !== -1) {
                    filename = filename.replace(resolveURL('/static/video-conference/'), '');
                    filename += resolveURL('/static/video-conference/index.html');
                } else if (filename.indexOf(resolveURL('/static')) !== -1) {
                    filename = filename.replace(resolveURL('/static/'), '');
                    filename = filename.replace(resolveURL('/static'), '');
                    filename += resolveURL('/static/index.html');
                } else {
